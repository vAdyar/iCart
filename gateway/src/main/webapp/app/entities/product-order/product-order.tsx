  ...
  return (
    <div>
      <h2 id="product-order-heading">
        <Translate contentKey="gatewayApp.productOrder.home.title">Product Orders</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="gatewayApp.productOrder.home.createLabel">Create new Product Order</Translate>
        </Link>
      </h2>
        ...
        {productOrderList && productOrderList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('customerId')}>
                  <Translate contentKey="gatewayApp.productOrder.customerId">Customer Id</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('orderItems')}>
                  <Translate contentKey="gatewayApp.productOrder.orderItems">Order Items</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('placedDate')}>
                  <Translate contentKey="gatewayApp.productOrder.placedDate">Placed Date</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('status')}>
                  <Translate contentKey="gatewayApp.productOrder.status">Status</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="gatewayApp.productOrder.code">Code</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="gatewayApp.productOrder.invoice">Invoice</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {productOrderList.map((productOrder, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{productOrder.orderItems}</td>
                  <td>
                    {productOrder.placedDate ? <TextFormat type="date" value={productOrder.placedDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    <Translate contentKey={`gatewayApp.OrderStatus.${productOrder.status}`} />
                  </td>
                  <td>{productOrder.code}</td>
                  <td>{productOrder.invoice ? <Link to={`invoice/${productOrder.invoice.id}`}>{productOrder.invoice.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="gatewayApp.productOrder.home.notFound">No Product Orders found</Translate>
            </div>
          )
        )}
      </div>
