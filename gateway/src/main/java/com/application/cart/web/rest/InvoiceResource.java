...
import com.application.cart.service.InvoiceService;
...
/**
 * REST controller for managing {@link com.application.cart.domain.Invoice}.
 */
@RestController
@RequestMapping("/api")
public class InvoiceResource {
     ...
     */
    @GetMapping("/invoices")
    public ResponseEntity<List<Invoice>> getAllInvoices(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("productorder-is-null".equals(filter)) {
            log.debug("REST request to get all Invoices where productOrder is null");
            return new ResponseEntity<>(invoiceService.findAllWhereProductOrderIsNull(),
                    HttpStatus.OK);
        }
